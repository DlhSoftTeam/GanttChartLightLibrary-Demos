...
Partial Public Class MainWindow
    ...
    Public Sub New()
        ...
        For i As Integer = 6 To 25
            start = If(i <= 8, (i - 4) * 2, i - 8)
            finish = (If(i <= 8, (i - 4) * 2 + (If(i > 8, 6, 1)), i - 2)) + 2
            GanttChartDataGrid.Items.Add(New GanttChartItem With {.Content = "Task " & i, .Indentation = If(i Mod 3 = 0, 0, 1), .Start = Date.Today.AddDays(start), .Finish = Date.Today.AddDays(finish), .CompletedFinish = Date.Today.AddDays(If(i <= 8, (i - 4) * 2, i - 8)).AddDays(If(i Mod 6 = 4, 3, 0)), .BaselineStart = Date.Today.AddDays(start - 2), .BaselineFinish = Date.Today.AddDays(finish - 2)})
        Next i
    End Sub
