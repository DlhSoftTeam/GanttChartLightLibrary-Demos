...
Partial Public Class MainWindow
    ...
    Public Sub New()
        ...
        Dim item0 As StatusGanttChartItem = TryCast(GanttChartDataGrid.Items(0), StatusGanttChartItem)

        Dim item1 As StatusGanttChartItem = TryCast(GanttChartDataGrid.Items(1), StatusGanttChartItem)
        ...
        Dim item2 As StatusGanttChartItem = TryCast(GanttChartDataGrid.Items(2), StatusGanttChartItem)
        ...
        Dim item3 As StatusGanttChartItem = TryCast(GanttChartDataGrid.Items(3), StatusGanttChartItem)
        item3.Predecessors.Add(New PredecessorItem With {.Item = item0, .DependencyType = DependencyType.StartStart})

        Dim item4 As StatusGanttChartItem = TryCast(GanttChartDataGrid.Items(4), StatusGanttChartItem)
        item4.Start = Date.Today.Add(TimeSpan.Parse("08:00:00"))
        item4.Finish = Date.Today.AddDays(2).Add(TimeSpan.Parse("12:00:00"))

        Dim item6 As StatusGanttChartItem = TryCast(GanttChartDataGrid.Items(6), StatusGanttChartItem)
        item6.Start = Date.Today.Add(TimeSpan.Parse("08:00:00"))
        item6.Finish = Date.Today.AddDays(3).Add(TimeSpan.Parse("12:00:00"))

        Dim item7 As StatusGanttChartItem = TryCast(GanttChartDataGrid.Items(7), StatusGanttChartItem)
        ...
        For i As Integer = 3 To 25
            GanttChartDataGrid.Items.Add(New StatusGanttChartItem With {.Content = "Task " & i, .Indentation = If(i Mod 3 = 0, 0, 1), .Start = Date.Today.AddDays(If(i <= 8, (i - 4) * 2, i - 8)), .Finish = Date.Today.AddDays((If(i <= 8, (i - 4) * 2 + (If(i > 8, 6, 1)), i - 2)) + 2), .CompletedFinish = Date.Today.AddDays(If(i <= 8, (i - 4) * 2, i - 8)).AddDays(If(i Mod 6 = 4, 3, 0))})
        Next i
