...
namespace Demos.WPF.CSharp.GanttChartDataGrid.WorkOptimizations
{
    /// <summary>
    ...
    public partial class MainWindow : Window
    {
            ...
            for (int i = 3; i <= 25; i++)
            {
                GanttChartDataGrid.Items.Add(
                    new GanttChartItem
                    {
                        ...
                        AssignmentsContent = i % 3 == 0 ? "Resource 2" : "Resource 1"
                    });
            }
        }
        ...
        {
            if (StartOnDatePicker.SelectedDate != null)
                GanttChartDataGrid.OptimizeWork((bool)DependenciesOnlyButton.IsChecked, (bool)IncludeStartedTasksButton.IsChecked, (DateTime)StartOnDatePicker.SelectedDate);
            else
                GanttChartDataGrid.OptimizeWork((bool)DependenciesOnlyButton.IsChecked, (bool)IncludeStartedTasksButton.IsChecked);
        }

        private void LevelResourcesButton_Click(object sender, RoutedEventArgs e)
        {
            if (StartOnDatePicker.SelectedDate != null)
                GanttChartDataGrid.LevelResources((bool)IncludeStartedTasksButton.IsChecked, (DateTime)StartOnDatePicker.SelectedDate);
            else
                GanttChartDataGrid.LevelResources((bool)IncludeStartedTasksButton.IsChecked);
        }
    }
}
